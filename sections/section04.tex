...
\begin{corollary}[Формулы Родрига]
	...
	\[
		\nabla_{\vec{\xi}_k}\vec{n} = -\lambda_k\vec{\xi}_k.
	\]
	Здесь через $\nabla_{\vec{\xi}_k}$ обозначена производная по направлению вектора $\vec{\xi}_k$.
\end{corollary}
...
\begin{proof}
	...
	\]
	Последнее равенство выполнено, так как вектор $\vec{\xi}_k$ является собственным для оператора Вайнагртена с собственным значением $-\lambda_k$.
\end{proof}
...
Отметим, что коммутатор задаёт на пространстве всех векторных полей на поверхности структуру бесконечномерной \textit{алгебры Ли}.
...
Итак, на поверхности отрицательной кривизны мы локально указали два векторных поля $\vec{e}_1$, $\vec{e}_2$ (с точностью до знака каждого из них и перестановки). Оказывается, что на поверхности постоянной отрицательной кривизны эти поля коммутируют, что мы сейчас и покажем. Но сначала докажем общее утверждение.
...
\footnotetext{Уравнение $\sin$-Гордон играет важную роль в теории солитонов. Я не понимаю, что это значит и не думаю, что должен, но об этом упоминается везде, где написано про это уравнение. Я продолжаю добрую традицию.}
