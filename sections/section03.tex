...
\begin{solution}
	...
	Обратим внимание на <<непорядочность>> принятых обозначений в пункте $(2)$ --- за $ds$ мы обозначили как единицу длины, так и дифференциал по натуральному параметру на данной кривой. Делать так, конечно же, нехорошо, но дальнейших вычислений у нас нет, поэтому конкретно здесь это не так важно.
\end{solution}
...
\[
	K = \frac{g_{11}g_{22} - g_{12}^2}{b_{11}b_{22} - b_{12}^2} = \frac{\det\G}{\det\B},\qquad H = \frac{g_{11}b_{22} + g_{22}b_{11} - 2g_{12}b_{12}}{g_{11}g_{22} - g_{12}^2}.
\]
...
Отбросив $\o(x^2 + y^2)$, мы получим уравнение параболоида, который приближает нашу поверхность вблизи начала координат. Эта соприкасающайся поверхность второго порядка служит аналогом соприкасающейся окружности к кривой.

Далее мы приведём очень явный геометрический смысл главных кривизн. Для этого мы будем рассматривать сечения поверхности плоскостями. 
...
В дальнейшем в этом разделе кривизна нормального сечения будет пониматься в контексте ориентированной кривизны.
...
Теперь рассмотрим произвольное нормальное сечение. Его однозначно можно задать углом $\varphi$ ($0 \leqslant \varphi < 2\pi$) относительно вектора $\vec{\xi}_1$.

\begin{figure}[H]
	\centering
	\includegraphics[width=5cm]{./img/NormalSection.pdf}
	\caption[format=empty]{}
\end{figure}

Можем ввести в плоскости $\span(\vec{\xi}_1, \vec{\xi}_2)$ полярные координаты $(\xi, \varphi)$ и переписать через них формулу \eqref{eq:TailorII}:
\[
	z = \frac{\lambda_1\cos^2\varphi + \lambda_2\sin^2\varphi}{2}\xi^2 + \o(\xi^2).
\]

Аналогично уже сделанным выкладкам, легко посчитать кривизну нормального сечения плоскостью $\span(\vec{n}, \vec{\xi})$.
...
\begin{theorem}[Формула Эйлера]
	...
	\]
	где $\lambda_1$ и $\lambda_2$ --- главные кривизны, а $\varphi$ --- угол между $\vec{\xi}$ и главным направлением $\vec{\xi}_1$ (соответствующим $\lambda_1$).
\end{theorem}

Положим для определённости $\lambda_1 \leqslant \lambda_2$. Тогда главные кривизны $\lambda_1$ и $\lambda_2$ --- минимум и максимум, соответственно, кривизн нормальных сечений в рассматриваемой точке.

Обратимся к предложению \ref{proposition:GeomII}. (Здесь также точками обозначены производые по $t$.) Обозначим через $\vec{n}_\rho$ вектор главной нормали кривой $\vec{\rho}$ в рассматриваемой точке, а через $\theta$ --- угол между ним и вектором нормали к поверхности, то есть $\theta = \angle(\vec{n}_\rho, \vec{n})$. Кривизна кривой $\vec{\rho}$ определяется из соотношения
\[
...
\[
	\left\langle\frac{d^2\vec{\rho}}{ds^2}, \vec{n}\right\rangle = b_{11}\dot{u}^2 + 2b_{12}\dot{u}\dot{v} + b_{22}\dot{v}^2 = \frac{b_{ij}\,dx^idx^j}{ds^2},
\]
причём, из определения натурального параметра, $ds^2 = \abs{\dot{\vec{\rho}}}dt^2$:
\[
